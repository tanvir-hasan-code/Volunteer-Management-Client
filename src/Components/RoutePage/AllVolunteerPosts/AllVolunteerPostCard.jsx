...
const AllVolunteerPostCard = ({ post, isTable }) => {
  const TableView = () => (
    <div className="w-full border-b py-3 px-2 flex  md:flex-row justify-between items-center md:items-center text-sm md:text-base">
      ...
      <div className="flex-1 font-semibold text-sm md:text-lg">{post.title}</div>
      <div className="flex-1 flex items-center gap-1 text-blue-300">
        <IoLocationSharp /> <small>{post.location}</small>
      </div>
      <div className="flex-1 text-xs md:text-sm">{post.volunteersNeeded} People</div>
      <div className="flex-1 text-xs md:text-sm">Deadline: {post.applicationDeadline}</div>
      <div className="flex-1 text-right mt-2 md:mt-0">
        ...
        >
          <span className="hidden md:block">View</span> Details <FaEye size={14} />
        </NavLink>
      </div>
    </div>
  );
