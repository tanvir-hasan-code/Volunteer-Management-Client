      ...
      <div className="w-full md:w-11/12 mx-auto my-12">
        ...
        <div className="flex flex-wrap justify-center gap-6">
          {testimonials.map((item, i) => (
            <motion.div
              key={item.id}
              className={`${theme === 'light'? "bg-white": "bg-gray-700 "} shadow-lg rounded-xl p-6 max-w-sm text-center`}
              initial={{ opacity: 0, y: 50 }}
              whileInView={{ opacity: 1, y: 0 }}
              transition={{ duration: 0.5, delay: i * 0.2 }}
              viewport={{ once: false, amount: 0.3 }}
            >
              <p className={` ${theme === "light"? "`text-gray-700": ""} mb-4`}>"{item.text}"</p>
              <h3 className={`font-semibold ${theme === 'light'? "text-gray-700": "text-blue-300"}`}>{item.name}</h3>
            </motion.div>
          ))}
        </div>
      </div>
